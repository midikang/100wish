...
class WishDatabase {
  ...
  async addWish(wish: Omit<Wish, 'id' | 'createdAt' | 'updatedAt'>): Promise<Wish> {
    const newWish: Wish = {
      ...wish,
  ...
  async updateWish(id: number, updates: Partial<Wish>): Promise<Wish | null> {
    ...
    return Promise.resolve({ ...this.wishes[index] });
  }
...
export const wishDb = new WishDatabase();